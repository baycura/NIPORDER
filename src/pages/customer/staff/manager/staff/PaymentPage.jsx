...
const METHODS = [
  { id:"cash",     label:"Nakit",    icon:"💵", color:"#3ECF8E" },
  { id:"card",     label:"Kart",     icon:"💳", color:"#5A8FE0" },
  { id:"transfer", label:"Havale",   icon:"📱", color:"#C8973E" },
  { id:"debt",     label:"Veresiye", icon:"📋", color:"#E05A5A" },
];
...
function PayModal({ order, onClose, onPaid, staffId }) {
  ...
  const handlePay = async () => {
    ...
    await supabase.from("payments").insert({
      order_id: order.id, method, amount: total, staff_id: staffId,
    });
    await supabase.from("orders").update({
      status: method === "debt" ? "debt" : "paid",
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0,
      background:"#000000bb", zIndex:200,
      display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div onClick={e => e.stopPropagation()} style={{ background:"#161616",
        border:"1px solid #2A2A2A", borderRadius:16, padding:28, width:380 }}>
        <div style={{ display:"flex", justifyContent:"space-between",
          alignItems:"center", marginBottom:20 }}>
          <div style={{ color:"#F0EDE8", fontFamily:cv, fontSize:22 }}>
            {order.cafe_tables?.name}
          </div>
          <div style={{ color:"#C8973E", fontFamily:cv, fontSize:32,
            letterSpacing:"-1px" }}>₺{total.toLocaleString()}</div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr",
          ...
          {METHODS.map(m => (
            <button key={m.id} onClick={() => setMethod(m.id)}
              style={{ padding:"12px", border:`2px solid ${method===m.id?m.color:"#2A2A2A"}`,
                background: method===m.id ? m.color+"22" : "transparent",
                color: method===m.id ? m.color : "#888",
                borderRadius:10, cursor:"pointer", fontFamily:cvc,
        ...
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={handlePay} disabled={loading}
            style={{ flex:2, padding:"12px",
              background: loading ? "#333" : "#3ECF8E",
              border:"none", color:"#000", fontFamily:cv, fontSize:18,
              letterSpacing:"0.5px", cursor:"pointer", borderRadius:8 }}>
            {loading ? "İŞLENİYOR..." : `TAHSİL ET · ₺${total.toLocaleString()}`}
          </button>
        </div>
      </div>
    </div>
...
export default function PaymentPage() {
  ...
  const totalOpen = orders.reduce((s, o) => s + (o.total || 0), 0);
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between",
        ...
        <h1 style={{ color:"#F0EDE8", fontFamily:cv, fontSize:28,
          letterSpacing:"-0.5px", margin:0 }}>Kasa</h1>
        <div style={{ color:"#3ECF8E", fontFamily:cvc, fontSize:13 }}>
          Açık: ₺{Math.round(totalOpen).toLocaleString()}
        </div>
      </div>
      ...
      {orders.length === 0 && !loading && (
        <div style={{ color:"#888", fontFamily:cvc, fontSize:13,
          textAlign:"center", padding:60 }}>Ödeme bekleyen sipariş yok</div>
      )}
        ...
        {orders.map(order => (
          <div key={order.id} style={{ background:"#1E1E1E",
            ...
            <div style={{ display:"flex", justifyContent:"space-between",
              ...
              <div style={{ color:"#C8973E", fontFamily:cv, fontSize:24,
                letterSpacing:"-0.5px" }}>
            ...
            <div style={{ marginBottom:12 }}>
              {(order.order_items || []).slice(0,3).map((item,i) => (
                <div key={i} style={{ color:"#888", fontFamily:cvc,
              ...
              {(order.order_items?.length || 0) > 3 && (
                <div style={{ color:"#444", fontFamily:cvc, fontSize:11 }}>
                  +{order.order_items.length - 3} daha
                </div>
              )}
            </div>
            <button onClick={() => setPayOrder(order)}
              style={{ width:"100%", padding:"10px", background:"#3ECF8E",
      ...
      {payOrder && (
        <PayModal order={payOrder} staffId={staffUser?.id}
          onClose={() => setPayOrder(null)} onPaid={load} />
      )}
    </div>
  );
}
