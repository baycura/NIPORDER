...
const MANAGER_NAV = [
  { to:"/stock-mgmt",  icon:"🗃️", label:"Stok Yönetimi" },
...
export default function StaffLayout() {
  ...
  return (
    <div style={{ display:"flex", background:"#0C0C0C", minHeight:"100vh" }}>
      ...
      {/* Sidebar */}
      <aside style={{ width:220, background:"#161616", borderRight:"1px solid #2A2A2A",
        ...
        <div style={{ padding:"18px 16px 14px", borderBottom:"1px solid #2A2A2A" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:36, height:36, borderRadius:10, background:"#C8973E",
              display:"flex", alignItems:"center", justifyContent:"center",
              fontFamily:cv, fontSize:20, color:"#000" }}>N</div>
            <div>
              <div style={{ color:"#F0EDE8", fontFamily:cv, fontSize:16 }}>NotInParis</div>
              <div style={{ color:"#888", fontFamily:cvc, fontSize:9, letterSpacing:"1px" }}>
                order.notinparis.me
              </div>
            </div>
          </div>
          ...
          {STAFF_NAV.map(item => (
            <NavLink key={item.to} to={item.to}
              style={({ isActive }) => ({
                ...
                background: isActive ? "rgba(200,151,62,0.12)" : "transparent",
                color: isActive ? "#C8973E" : "#888",
                fontFamily:cvc, fontSize:12, letterSpacing:"0.5px",
              })}>
        ...
        <div style={{ padding:"12px", borderTop:"1px solid #2A2A2A" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:32, height:32, borderRadius:"50%",
              ...
              justifyContent:"center", color, fontFamily:cv, fontSize:16 }}>
              {staffUser?.name?.[0] || "?"}
            </div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ color:"#F0EDE8", fontFamily:cvc, fontSize:12, fontWeight:700 }}>
                {staffUser?.name}
              </div>
              <div style={{ color, fontFamily:cvc, fontSize:9, letterSpacing:"1px" }}>
