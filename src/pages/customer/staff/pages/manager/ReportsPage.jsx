...
export default function ReportsPage() {
    ...
    const load = async () => {
      ...
      (data || []).forEach(o =>
        (o.order_items || []).forEach(item => {
          ...
          map[item.product_name].revenue += item.final_price * item.quantity;
        })
      );
  ...
  const byDay = {};
  orders.forEach(o => {
    const day = new Date(o.paid_at).toLocaleDateString("tr",
  ...
  const STATS = [
    ...
    ["SİPARİŞ",   orders.length.toString(),                          "#5A8FE0", "📋"],
    ["ORT. SEPET",`₺${Math.round(avgOrder).toLocaleString()}`,      "#C8973E", "📊"],
    ["İNDİRİM",   `₺${Math.round(totalDiscount).toLocaleString()}`, "#E05A5A", "🏷️"],
  ];
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between",
        ...
        <h1 style={{ color:"#F0EDE8", fontFamily:cv, fontSize:28,
          letterSpacing:"-0.5px", margin:0 }}>Raporlar</h1>
        <div style={{ display:"flex", gap:6 }}>
          {[["week","7 Gün"],["month","30 Gün"],["quarter","90 Gün"]].map(([id,lbl]) => (
            <button key={id} onClick={() => setPeriod(id)}
              style={{ padding:"6px 14px", borderRadius:8, border:"none",
                fontFamily:cvc, fontSize:11, letterSpacing:"1px", cursor:"pointer",
                background: period===id ? "#C8973E" : "transparent",
                color:       period===id ? "#000"   : "#888",
                outline:     period!==id ? "1px solid #2A2A2A" : "none" }}>
              {lbl}
            </button>
          ))}
        </div>
          ...
          <div key={label} style={{ background:"#1E1E1E",
            ...
            <div style={{ display:"flex", justifyContent:"space-between" }}>
              <div>
                ...
                <div style={{ color, fontFamily:cv, fontSize:24,
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1.6fr 1fr", gap:16 }}>
        ...
        <div style={{ background:"#1E1E1E", border:"1px solid #2A2A2A",
          ...
          <div style={{ color:"#F0EDE8", fontFamily:cv, fontSize:16,
            marginBottom:16 }}>Günlük Ciro</div>
          {loading ? (
            <div style={{ color:"#888", fontFamily:cvc, fontSize:12,
              textAlign:"center", padding:20 }}>YÜKLENİYOR...</div>
          ) : dayEntries.length === 0 ? (
            <div style={{ color:"#888", fontFamily:cvc, fontSize:12,
              textAlign:"center", padding:20 }}>Veri yok</div>
          ) : (
            <div style={{ display:"flex", alignItems:"flex-end",
              gap:8, height:160 }}>
              {dayEntries.map(([day,val], i) => (
                <div key={day} style={{ flex:1, display:"flex",
                  flexDirection:"column", alignItems:"center",
                  gap:4, height:"100%", justifyContent:"flex-end" }}>
                  <div style={{ color:"#888", fontFamily:cvc, fontSize:9 }}>
                    ₺{(val/1000).toFixed(1)}k
                  </div>
                  <div style={{ width:"100%", borderRadius:"4px 4px 0 0",
                    minHeight:4, transition:"height 0.5s",
                    background: i===dayEntries.length-1 ? "#C8973E" : "#C8973E44",
        ...
        <div style={{ background:"#1E1E1E", border:"1px solid #2A2A2A",
          borderRadius:12, padding:20 }}>
          <div style={{ color:"#F0EDE8", fontFamily:cv, fontSize:16,
            marginBottom:14 }}>En Çok Satan</div>
          {topItems.length === 0 && (
            <div style={{ color:"#888", fontFamily:cvc, fontSize:12,
              textAlign:"center", padding:16 }}>Veri yok</div>
          )}
          {topItems.map((item, i) => (
            <div key={i} style={{ marginBottom:12 }}>
              <div style={{ display:"flex", justifyContent:"space-between",
                ...
                <span style={{ color:"#F0EDE8", fontFamily:cvc,
                  fontSize:12 }}>{item.name}</span>
                <span style={{ color:"#C8973E", fontFamily:cvc,
                  fontSize:12 }}>{item.qty} adet</span>
              </div>
              <div style={{ background:"#2A2A2A", borderRadius:3, height:3 }}>
                <div style={{ background:"#C8973E", height:"100%",
                  borderRadius:3,
