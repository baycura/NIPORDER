...
function EntryModal({item,staffId,onClose,onDone}){
  ...
  return(<div onClick={onClose} style={{position:"fixed",inset:0,background:"#000000bb",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center"}}><div onClick={e=>e.stopPropagation()} style={{background:"#161616",border:"1px solid #2A2A2A",borderRadius:16,padding:28,width:360}}>
    ...
    <input type="number" value={qty} onChange={e=>setQty(e.target.value)} placeholder={`Miktar (${item.unit})`} style={{width:"100%",background:"#111",border:"1px solid #2A2A2A",borderRadius:8,padding:"11px 14px",color:"#F0EDE8",fontFamily:cvc,fontSize:16,marginBottom:12}}/>
    ...
    <div style={{display:"flex",gap:10}}><button onClick={onClose} style={{flex:1,padding:"11px",background:"transparent",border:"1px solid #2A2A2A",color:"#888",borderRadius:8,cursor:"pointer",fontFamily:cvc,fontSize:12}}>İptal</button><button onClick={save} disabled={saving} style={{flex:2,padding:"11px",background:saving?"#333":"#3ECF8E",border:"none",color:"#000",borderRadius:8,cursor:saving?"wait":"pointer",fontFamily:cv,fontSize:16}}>{saving?"KAYDEDİLİYOR...":"STOKA EKLE"}</button></div>
  </div></div>);}
export default function StockViewPage(){
  const{staffUser}=useAuth();const[items,setItems]=useState([]);const[loading,setLoading]=useState(true);const[entry,setEntry]=useState(null);const[search,setSearch]=useState("");
  const load=async()=>{const{data}=await supabase.from("stock_items").select("*").order("name");setItems(data||[]);setLoading(false);};
  useEffect(()=>{load();},[]);
  ...
  return(<div>
    ...
    {alerts.length>0&&<div style={{background:"rgba(224,90,90,0.12)",border:"1px solid #E05A5A",borderRadius:10,padding:"10px 16px",marginBottom:16,display:"flex",gap:10,alignItems:"center"}}><span>⚠️</span><span style={{color:"#E05A5A",fontFamily:cvc,fontSize:12}}>{alerts.length} ürün kritik</span></div>}
    ...
    <div style={{background:"#1E1E1E",border:"1px solid #2A2A2A",borderRadius:12,overflow:"hidden"}}>
      {filtered.map((item,i)=>{const lvl=alertLevel(item);const color=AC[lvl];return(<div key={item.id} style={{display:"grid",gridTemplateColumns:"1.8fr 1fr 1fr .8fr",padding:"11px 16px",alignItems:"center",borderBottom:i<filtered.length-1?"1px solid #2A2A2A":"none"}}>
