...
export default function StaffLayout() {
  ...
  const linkStyle = (isActive) => ({
    ...
    color:isActive?"#C8973E":"#aaa",
    fontFamily:cv,fontSize:14,fontWeight:isActive?700:500,
  });

  const NavItem = ({item, onClick}) => (
    <NavLink to={item.to} onClick={onClick} style={({isActive}) => linkStyle(isActive)}>
    ...
    <div style={{
      ...
      <div style={{padding:14,borderTop:"1px solid #2A2A2A",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{color,fontSize:10,letterSpacing:"1px",fontFamily:cv,fontWeight:600}}>{(roleLabel[staffUser?.role]||staffUser?.role||"").toUpperCase()}</div>
        </div>
